...
/* Helper: GET text from URL (follow redirects) */
...
/* Helper: parse JSON safely, including cases where it's quoted twice */
...
/** Map a campaign to the unified card shape */
function mapCampaign(c) {
  ...
  return {
    ...
    updated_at:
      cleanStr(c.updated_at) ||
      cleanStr(c.end_date) ||
      cleanStr(c.created_at) ||
      '',
  };
}
...
/* Use UTC stamp for versioned filenames */
function utcStamp() {
  const d = new Date();
...
(async function main() {
  try {
    ...
    // 2) Flexible shapes
    const rawCampaigns = Array.isArray(campaignsRoot?.front)
      ? campaignsRoot.front
      : Array.isArray(campaignsRoot?.campaigns)
      ? campaignsRoot.campaigns
      : Array.isArray(campaignsRoot)
      ? campaignsRoot
      : [];

    const rawUpdates = Array.isArray(updatesRoot?.updates)
      ? updatesRoot.updates
      : Array.isArray(updatesRoot)
      ? updatesRoot
      : [];
    ...
    // 8) Build small index and home lists
